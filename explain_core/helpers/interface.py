...
class Interface:
    ...
    def plot_vitals(self, time):
        self.plot_time_graph(["AA.systole", "AA.diastole", "ecg.heart_rate", "breathing.spont_resp_rate", "AA.so2"], time_to_calculate=time, combined=True, sharey=False, autoscale=False, ylowerlim=0, yupperlim=200, fill=False, fill_between=True)
    ...
    def plot_lung_pressures(self, time=10, combined=True, sharey=True, autoscale=True, ylowerlim=0, yupperlim=100, fill=False, analyze=False):
        self.plot_time_graph(["NCA.pres","ALL.pres","ALR.pres"], time_to_calculate=time, autoscale=True, combined=combined, sharey=sharey, sampleinterval = 0.0005, ylowerlim=ylowerlim, yupperlim=yupperlim, fill=fill, fill_between=False,analyze=analyze)
    ...
    def plot_lung_flows(self, time=10, combined=True, sharey=True, autoscale=True, ylowerlim=0, yupperlim=100, fill=False, analyze=False):
        self.plot_time_graph(["OUT_NCA.flow","NCA_ALL.flow","NCA_ALR.flow"], time_to_calculate=time, autoscale=True, combined=combined, sharey=sharey, sampleinterval = 0.0005, ylowerlim=ylowerlim, yupperlim=yupperlim, fill=fill, fill_between=False, analyze=analyze)
    ...
    def plot_lung_pressures_left(self, time=10, combined=True, sharey=True, autoscale=True, ylowerlim=0, yupperlim=100, fill=False, analyze=False):
        self.plot_time_graph(["NCA.pres","ALL.pres"], time_to_calculate=time, autoscale=True, combined=combined, sharey=sharey, sampleinterval = 0.0005, ylowerlim=ylowerlim, yupperlim=yupperlim, fill=fill, fill_between=False, analyze=analyze)
    ...
    def plot_lung_flows_left(self, time=10, combined=True, sharey=True, autoscale=True, ylowerlim=0, yupperlim=100, fill=False, analyze=False):
        self.plot_time_graph(["OUT_NCA.flow","NCA_ALL.flow"], time_to_calculate=time, autoscale=True, combined=combined, sharey=sharey, sampleinterval = 0.0005, ylowerlim=ylowerlim, yupperlim=yupperlim, fill=fill, fill_between=False, analyze=analyze)
    ...
    def plot_lung_pressures_right(self, time=10, combined=True, sharey=True, autoscale=True, ylowerlim=0, yupperlim=100, fill=False, analyze=False):
        self.plot_time_graph(["NCA.pres","ALR.pres"], time_to_calculate=time, autoscale=True, combined=combined, sharey=sharey, sampleinterval = 0.0005, ylowerlim=ylowerlim, yupperlim=yupperlim, fill=fill, fill_between=False, analyze=analyze)
    ...
    def plot_lung_flows_right(self, time=10, combined=True, sharey=True, autoscale=True, ylowerlim=0, yupperlim=100, fill=False, analyze=False):
        self.plot_time_graph(["OUT_NCA.flow","NCA_ALR.flow"], time_to_calculate=time, autoscale=True, combined=combined, sharey=sharey, sampleinterval = 0.0005, ylowerlim=ylowerlim, yupperlim=yupperlim, fill=fill, fill_between=False, analyze=analyze)
    ...
    def get_vitals(self, time_to_calculate = 10):
        vitals = {
            "heartrate": self.model.components['ecg'].heart_rate,
            "spo2_pre": self.model.components['AA'].so2,
            "abp_systole": self.model.components['AA'].systole,
            "abp_diastole": self.model.components['AA'].diastole,
            "pap_systole": self.model.components['PA'].systole,
            "pap_diastole": self.model.components['PA'].diastole,
            "cvp": (2 * self.model.components['RA'].diastole + self.model.components['RA'].systole) / 3,
            "resp_rate": self.model.components['breathing'].spont_resp_rate
        }
    ...
    def get_blood_gas(self, component = 'AA'):
        ...
        if (component_type == "BloodCompliance" or component_type == "TimeVaryingElastance") and component_content == "blood":
            ...
            self.model.components['blood'].acidbase(self.model.components[component])
            self.model.components['blood'].oxygenation(self.model.components[component])
            
            # build the bloodgas dictionnary
            bg['ph'] = self.model.components[component].ph
            bg['po2'] = self.model.components[component].po2
            bg['pco2']= self.model.components[component].pco2
            bg['hco3'] = self.model.components[component].hco3
            bg['be'] = self.model.components[component].be
            bg['so2'] = self.model.components[component].so2
    ...
    def get_total_blood_volume(self):
        ...
        for component in self.model.components:
            ...
            if component == 'RLB' or component == 'AAR' or component == 'AA' or component == 'AD' or component == 'IVCI' or component == 'IVCE' or component == 'INT' or component == 'LS' or component == 'KID':
                lower_body+= self.model.components[component].vol
                
            
                
        syst_blood_volume = total_volume - pulm_blood_volume
        
        print (f"Total blood volume: {total_volume * 1000 / self.model.weight} ml/kg = {total_volume / total_volume * 100}%")
        print (f"Systemic blood volume: {syst_blood_volume * 1000 / self.model.weight} ml/kg = {syst_blood_volume / total_volume * 100}%")
        print (f"Pulmonary total blood volume: {pulm_blood_volume * 1000 / self.model.weight} ml/kg = {pulm_blood_volume / total_volume * 100}%")
        print (f"Pulmonary capillary blood volume: {pulm_cap_volume * 1000 / self.model.weight} ml/kg = {pulm_cap_volume / pulm_blood_volume * 100}% of total pulmonary blood volume")
        print (f"Heart blood volume: {heart_volume * 1000 / self.model.weight} ml/kg = {heart_volume / total_volume * 100}%")
        print (f"Capillary blood volume: {cap_volume * 1000 / self.model.weight} ml/kg = {cap_volume / total_volume * 100}%")
        print (f"Venous blood volume: {ven_volume * 1000 / self.model.weight} ml/kg = {ven_volume / total_volume * 100}%")
        print (f"Arterial blood volume: {art_volume * 1000 / self.model.weight} ml/kg = {art_volume / total_volume * 100}%")
        print (f"Upper body blood volume: {upper_body * 1000 / self.model.weight} ml/kg = {upper_body / total_volume * 100}%")
        print (f"Lower body blood volume: {lower_body * 1000 / self.model.weight} ml/kg = {lower_body / total_volume * 100}%")
    ...
    def inspect_model_component(self, component):
        content = {}
        for attribute in dir(self.model.components[component]):
            if not attribute.startswith('__'):
                attr_type = type(getattr(self.model.components[component],attribute))
                if (attr_type is str) or (attr_type is float) or (attr_type is bool) or (attr_type is int):
                    content[attribute] = getattr(self.model.components[component], attribute)
        return content
        ...
        for idx, parameter in enumerate(parameters):
          ...
          if prop_category[1] == "flow":
            ...
            if (sampleinterval != self.model.modeling_stepsize):
                ...
                bpm = self.model.components['ecg'].heart_rate
            sv = round(flow / bpm, 5)
