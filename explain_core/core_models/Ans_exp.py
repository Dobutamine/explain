...
class Ans_exp:
    ...
    def initialize(self):
        ...
        for pathway in self.afferent_pathways:
...
class Pathway:
    def __init__(self, model, update_interval, pathway_args):
        ...
        self.setpoint = pathway_args["setpoint"]
        self.amplitude = pathway_args["amplitude"]
        self.sensitivity = pathway_args["sensitivity"]
        self.time_constant = pathway_args["time_constant"]
        self.efferent = pathway_args["efferent"]
    ...
    def calculate_pathway(self):
        ...
        # apply the effect
        if (self.is_enabled):
            return (self.efferent, self.net_effect)
        else:
            return (self.efferent, 0.0)
    ...
    def activation_function(self, value):
        activation = ((2 * self.amplitude) / (1 + math.pow(math.e,
                      (value - self.setpoint) * -self.sensitivity))) - self.amplitude
