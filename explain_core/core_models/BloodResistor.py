...
class BloodResistor:
    def __init__(self, model, **args):
        ...
        self.r_for = 1.0          # holds the resistance if the flow direction is from comp_from to comp_to
        self.r_for_fac = 1.0      # holds the factor with which r_for is multiplied
        self.r_back = 1.0         # holds the resistance if the flow direction is from comp_to to comp_from
        self.r_back_fac = 1.0     # holds the factor with which r_for is multiplied
        self.r_k = 0              # holds the constant for the non-linear flow dependency of the resistance
        self.r_k_fac = 1.0        # holds factor for the constant for the non-linear flow dependency of the resistance
        self.flow = 0
    ...
    def calculate_resistance(self, p1, p2):
        ...
        nonlin_fac = self.r_k * self.r_k_fac * abs(self.flow)
